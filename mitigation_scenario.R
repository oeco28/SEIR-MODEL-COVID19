...
###########################################################
#########################################
## Scenario of mitigation with relaxation
#########################################
###########################################################


mitigation2 <- data.frame(matrix(NA,500,7))
for(i in 1:500){
mitigation2[i,1] <- 1 - i*0.001
}

colnames(mitigation2) <- c("contact","Infected_all","Deceased_all","Infected_lowOnly","Deceased_lowOnly","Infected_highOnly","Deceased_highOnly")
...
parameters <- c(t_I_H <- 1 - c1,
        ...
        sigma_H = 0.192,      #1/5.2
        sigma_L = 0.192,
        gamma_H = 0.0807,     #1/17.16 ceiling or an average of 1/12.39=0.0807
        gamma_L = 0.0807,
        mu_H = 0.002,         #0.0004/.2
        mu_L = 0.0004)
...
out_1 <- ode(y = state, times = times, func = seir_2_risk_covid, parms = parameters)
colnames(out_1) <- c("time","Susceptible_H","Susceptible_L","Exposed_H","Exposed_L","Infected_H","Infected_L", "Recovered_H","Recovered_L","Deceased_H","Deceased_L")

out1 <- data.frame(out_1)

state2 <- c(S_H = out1[121,2],
    S_L = out1[121,3],
    E_H = out1[121,4],
    E_L = out1[121,5],
    I_H = out1[121,6],
...
times2 <- seq(0,320, by = 1)
out_2 <- ode(y = state2, times = times2, func = seir_2_risk_covid, parms = parameters2)
colnames(out_2) <- c("time","Susceptible_H","Susceptible_L","Exposed_H","Exposed_L","Infected_H","Infected_L", "Recovered_H","Recovered_L","Deceased_H","Deceased_L")

out2 <- data.frame(out_2)

state3 <- c(S_H = out2[121,2],
    S_L = out2[121,3],
    E_H = out2[121,4],
    E_L = out2[121,5],
    I_H = out2[21,6],
...
times3 <- seq(0,120, by = 1)
out_3 <- ode(y = state3, times = times3, func = seir_2_risk_covid, parms = parameters3)
colnames(out_3) <- c("time","Susceptible_H","Susceptible_L","Exposed_H","Exposed_L","Infected_H","Infected_L", "Recovered_H","Recovered_L","Deceased_H","Deceased_L")


out3 <- data.frame(out_3)

multi_phase1 <- rbind(out1,out2[-1,],out3[-1,])
multi_phase1$time <- seq(1,120*2 + 321,1)


plot(multi_phase1$time, (multi_phase1$Infected_H + multi_phase1$Infected_L) , type="l", lwd=2, xlab = "time (days)", ylab = "Number of infected individuals", col=rgb(0,0,0))
lines(-mitigation$contact, mitigation$Infected_lowOnly, lwd=2, col="blue")
lines(-mitigation$contact, mitigation$Infected_highOnly, lwd=2, col="red")
abline(v = 120, col="red", lwd=2, lty=3)
abline(v = 240, col="darkgreen", lwd=2, lty=3)
text(-5,6700,paste((1-c1),"*beta",sep=""),cex=0.7,col="black",pos=4)
text(122,8000,paste((1-c2),"*beta",sep=""),cex=0.7,col="red",pos=4)
text(242,8000,paste((1-c3),"*beta",sep=""),cex=0.7,col="darkgreen",pos=4)
